...
;
    ...
    function update_4900(){
        $sql[] = "CREATE OR REPLACE VIEW `swete_strings` AS
            select  s.string_id,
                    ...
                    t.normalized_translation_value,
                    if(tml.webpage_id is null,hrl.proxy_request_url, concat(ws.website_url,w.webpage_url)) as request_url,
                    ifnull(tml.translation_memory_id,tms.translation_memory_id) as translation_memory_id,
                    tm.translation_memory_uuid,
                    tm.translation_memory_name,
                    tm.source_language,
                    tm.destination_language,
    ...
    function update_4903(){
        $sql[] = "CREATE OR REPLACE VIEW `swete_strings` AS
            select  s.string_id,
                    ...
                    left join http_request_log hrl on tml.http_request_log_id=hrl.http_request_log_id
                    left join webpages w on tml.webpage_id=w.webpage_id
                    left join websites ws on (tml.website_id=ws.website_id OR tms.translation_memory_id=ws.translation_memory_id)
                    where ifnull(tml.translation_memory_id,tm.translation_memory_id) IS NOT NULL
            ";
    ...
    function update_4904(){
        $sql[] = "CREATE OR REPLACE VIEW `swete_strings` AS
            select  s.string_id,
                    ...
                    left join webpages w on tml.webpage_id=w.webpage_id
                    left join websites ws on (tml.website_id=ws.website_id OR tms.translation_memory_id=ws.translation_memory_id)
                    where ifnull(tml.translation_memory_id,tms.translation_memory_id) IS NOT NULL
                    and (tms.translation_memory_id IS NULL OR tms.current_translation_id IS NOT NULL)
            ";
    ...
    function update_4911(){
        $sql[] = "CREATE TABLE swete_strings_blacklist (string_id INT(11) NOT NULL PRIMARY KEY)";
        $sql[] = "CREATE OR REPLACE VIEW `swete_strings` AS
            select  s.string_id,
                    s.normalized_value as normalized_string,
                    s.normalized_value as string,
                    ...
                    left join webpages w on tml.webpage_id=w.webpage_id
                    left join websites ws on (tml.website_id=ws.website_id OR tms.translation_memory_id=ws.translation_memory_id)
                    left join swete_strings_blacklist bl on s.string_id=bl.string_id
                    where ifnull(tml.translation_memory_id,tms.translation_memory_id) IS NOT NULL
                    and (tms.translation_memory_id IS NULL OR tms.current_translation_id IS NOT NULL)
                    and bl.string_id IS NULL
            ";
		...
		function update_4950() {
				$sql[] = "alter table websites add column current_snapshot_id int(11)";
