	...
	function handle($params){
		...
		*/
		...
		foreach ($reader as $row){
			$results[] = $row;
			$row->untranslated_words = intval(@$row->numwords)-intval(@$row->translated_words);
			$row->untranslated_phrases = intval(@$row->numphrases)-intval(@$row->translated_phrases);
		...
		list($numPages) = xf_db_fetch_row($res);
		...
		list($numPhrases, $numWords) = xf_db_fetch_row($res);
		@xf_db_free_result($res);

		$res = df_q("select count(*) from websites");
		list($numSites) = xf_db_fetch_row($res);
		@xf_db_free_result($res);
		...
		$res = df_q("select count(*) as numphrases, ifnull(sum(xts.num_words),0) as num_words from swete_strings tml
			...
			where xttms.current_translation_id is not null");
		list($translatedPhrases, $translatedWords) = xf_db_fetch_row($res);
		@xf_db_free_result($res);
